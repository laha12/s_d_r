...
for run in get_tcp_run_list():
    ...
    if run.get("algorithm_label") == "ucb":
        debug_env = (
            "UCB_ROUTE_DEBUG_PATH='../../paper/ns3_experiments_ucb_iridium_66_6/traffic_matrix_poisson/"
            + logs_ns3_dir
            + "/ucb_route_debug.txt' "
        )
    command = (
        ...
        + debug_env +
        "LD_LIBRARY_PATH=build/optimized/lib "
        "./build/optimized/scratch/main_satnet/main_satnet "
        "--run_dir='../../paper/ns3_experiments_ucb_iridium_66_6/traffic_matrix_poisson/runs/" + run["name"] + "' "
        "2>&1 | tee '../../paper/ns3_experiments_ucb_iridium_66_6/traffic_matrix_poisson/" + logs_ns3_dir + "/console.txt'\""
    )
