...
def get_template_for_routing(routing_mode):
    if routing_mode == "ucb_distributed":
        return "templates/template_tcp_tm_config_ns3.properties"
    elif routing_mode == "spf":
        return "templates/template_tcp_tm_config_ns3_spf.properties"
    elif routing_mode == "elb":
        return "templates/template_tcp_tm_config_ns3_elb.properties"
    else:
        return "templates/template_tcp_tm_config_ns3.properties"
...
for run in get_tcp_run_list():
    ...
    routing_mode = run["satellite_network_routing"]
    template_path = get_template_for_routing(routing_mode)
    local_shell.copy_file(template_path, run_dir + "/config_ns3.properties")
    ...
    if routing_mode == "ucb_distributed":
        ...
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[UCB-NEGATIVE-REWARD]", str(run["ucb_negative_reward"]))
        ...
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[SPF-QUEUE-DROP-THRESHOLD]", str(run["spf_queue_drop_threshold"]))
    elif routing_mode == "elb":
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-QUEUE-DROP-THRESHOLD]", str(run["elb_queue_drop_threshold"]))
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-ALPHA-THRESHOLD]", str(run["elb_alpha_threshold"]))
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-BETA-THRESHOLD]", str(run["elb_beta_threshold"]))
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-CHI-REDUCTION-RATIO]", str(run["elb_chi_reduction_ratio"]))
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-AVG-PACKET-SIZE]", str(run["elb_avg_packet_size"]))
        local_shell.sed_replace_in_file_plain(run_dir + "/config_ns3.properties", "[ELB-RECOVERY-TIME-S]", str(run["elb_recovery_time_s"]))
