...
def main():
    ...
    sorted_items = sorted(
        events_by_key.items(),
        key=lambda x: (int(x[0][0]), x[0][1], x[0][2])
    )

    with open(summary_out, "w", encoding="utf-8") as f:
        f.write("uid\tsrc\tdst\tstatus\tevents\tfwd\tdrop\tarrive\tforward_to_dst\tfirst_line\tlast_line\n")
        for (uid, src, dst), events in sorted_items:
            ...
            f.write(
                f"{uid}\t{src}\t{dst}\t{stat}\t{event_count}\t{fwd_count}\t{drop_count}\t{arrive_count}\t{forward_to_dst_count}\t{first_line}\t{last_line}\n"
            )
    ...
    with open(grouped_out, "w", encoding="utf-8") as f:
        for (uid, src, dst), events in sorted_items:
            ...
            f.write(
                f"=== UID {uid} src={src} dst={dst} status={stat} events={len(events)} fwd={fwd_count} drop={drop_count} arrive={arrive_count} forward_to_dst={forward_to_dst_count} ===\n"
            )
            for e in events:
